...
func main() {
	...
	DBconnectionString := cfg.Database.ConnectionString(cfg.Database.ReplicaHost)
	...
	srv := server.NewReadServer(cfg, db, cacheProvider, rateLimiter, obs, pub)

	// Start read server in a goroutine
	go func() {
		obs.Logger.Info("Read Server starting",
			slog.String("port", cfg.ReadServer.Port),
	...
	if err := srv.Shutdown(shutdownCtx); err != nil {
		log.Fatalf("Read Server forced to shutdown: %v", err)
	}

	obs.Logger.Info("Read Server exited gracefully")
}
